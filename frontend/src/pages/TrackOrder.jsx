...
function TrackOrder() {
  ...
  return (
    ...
    {currentOrder?.shopOrders?.map((shopOrder,ind)=>(
      <div className='bg-white p-4 rounded-2xl shadow-md border border-orange-100 space-y-4' key={ind}>
        <div>
        ...
        {shopOrder.assignedDeliveryBoy?<div className='text-sm text-gray-700'>
            ...
            <p className='font-semibold'><span>DeliveryBoy contact No:</span> {shopOrder.assignedDeliveryBoy.mobile}</p>
        </div>:<p className='font-semibold'> Delivery Boy is not assigned yet.</p>}
        </>:<p className='text-green-600 font-semibold text-lg'>Delivered</p>}

        {shopOrder.assignedDeliveryBoy && shopOrder.status!=="delivered"  &&<div className='h-100 w-full rounded-2xl overflow-hidden shadow-md'><DeliveryBoyTracking data={{
          deliveryBoyLocation:{lon:shopOrder.assignedDeliveryBoy.location.coordinates[0],lat:shopOrder.assignedDeliveryBoy.location.coordinates[1]},
