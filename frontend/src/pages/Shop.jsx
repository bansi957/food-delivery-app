...
function Shop() {
  ...
  return (
    <div className='min-h-screen bg-gray-50 '>
      <button  className='absolute top-4 left-4 z-20  flex items-center gap-4 bg-black/50 hover:bg-black/70 cursor-pointer text-white px-3 py-2 rounded-full shadow-md transition' onClick={()=>navigate("/")}>
      <FaArrowLeft />
      <span>Back</span>
      </button>
        {shop && <div className='relative w-full h-64 md:h-80 lg:h-96'>
            ...
            <p className='text-lg font-medium text-gray-200 mt-2.5'> {shop.address}</p>
            </div>
            </div>
            </div>
            }
        <div className='max-w-7xl mx-auto px-6 py-10 '>
            ...
            {items?.length>0 ?<div className='flex flex-wrap justify-center gap-8'>
                {items.map((item,ind)=>(<FoodCard data={item} key={ind}/>))}
            </div>:<p className='text-center text-gray-500 text-lg'>No Items Available</p>}
